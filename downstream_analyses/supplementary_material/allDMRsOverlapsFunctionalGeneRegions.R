...
DMRs_overlap <- function(hal_CG_all,
  ...
  CG_hal_data <- data.frame(category = c("CDS", 
                            ...
                            count = c(sum(overlaps_hal_CG_final$context=="CDS"),
                                      sum(overlaps_hal_CG_final$context=="stop_codon"),
                                      sum(overlaps_hal_CG_final$context=="start_codon"),
                                      sum(overlaps_hal_CG_final$context=="transcription_end_site"),
                                      sum(overlaps_hal_CG_final$context=="intron"),
                                      sum(overlaps_hal_CG_final$context=="exon"),
                                      sum(overlaps_hal_CG_final$context=="TSS")))
  ...
  CHG_hal_data <- data.frame(category = c("CDS", 
                            ...
                            count = c(sum(overlaps_hal_CHG_final$context=="CDS"),
                                      sum(overlaps_hal_CHG_final$context=="stop_codon"),
                                      sum(overlaps_hal_CHG_final$context=="start_codon"),
                                      sum(overlaps_hal_CHG_final$context=="transcription_end_site"),
                                      sum(overlaps_hal_CHG_final$context=="intron"),
                                      sum(overlaps_hal_CHG_final$context=="exon"),
                                      sum(overlaps_hal_CHG_final$context=="transcription_start_site")))
  ...
  CHH_hal_data <- data.frame(category = c("CDS", 
                            ...
                            count = c(sum(overlaps_hal_CHH_final$context=="CDS"),
                                      sum(overlaps_hal_CHH_final$context=="stop_codon"),
                                      sum(overlaps_hal_CHH_final$context=="start_codon"),
                                      sum(overlaps_hal_CHH_final$context=="transcription_end_site"),
                                      sum(overlaps_hal_CHH_final$context=="intron"),
                                      sum(overlaps_hal_CHH_final$context=="exon"),
                                      sum(overlaps_hal_CHH_final$context=="transcription_start_site")))
  ...
  CG_lyr_data <- data.frame(category = c("CDS", 
                            ...
                            count = c(sum(overlaps_lyr_CG_final$context=="CDS"),
                                      sum(overlaps_lyr_CG_final$context=="stop_codon"),
                                      sum(overlaps_lyr_CG_final$context=="start_codon"),
                                      sum(overlaps_lyr_CG_final$context=="transcription_end_site"),
                                      sum(overlaps_lyr_CG_final$context=="intron"),
                                      sum(overlaps_lyr_CG_final$context=="exon"),
                                      sum(overlaps_lyr_CG_final$context=="transcription_start_site")))
  ...
  CHG_lyr_data <- data.frame(category = c("CDS", 
                             ...
                             count = c(sum(overlaps_lyr_CHG_final$context=="CDS"),
                                       sum(overlaps_lyr_CHG_final$context=="stop_codon"),
                                       sum(overlaps_lyr_CHG_final$context=="start_codon"),
                                       sum(overlaps_lyr_CHG_final$context=="transcription_end_site"),
                                       sum(overlaps_lyr_CHG_final$context=="intron"),
                                       sum(overlaps_lyr_CHG_final$context=="exon"),
                                       sum(overlaps_lyr_CHG_final$context=="transcription_start_site")))
  ...
  CHH_lyr_data <- data.frame(category = c("CDS", 
                             ...
                             count = c(sum(overlaps_lyr_CHH_final$context=="CDS"),
                                       sum(overlaps_lyr_CHH_final$context=="stop_codon"),
                                       sum(overlaps_lyr_CHH_final$context=="start_codon"),
                                       sum(overlaps_lyr_CHH_final$context=="transcription_end_site"),
                                       sum(overlaps_lyr_CHH_final$context=="intron"),
                                       sum(overlaps_lyr_CHH_final$context=="exon"),
                                       sum(overlaps_lyr_CHH_final$context=="transcription_start_site")))
  ...
  hal_anno <- data.frame(category = c("CDS", 
                            ...
                            count = c(sum(Ahal_anno$context=="CDS"),
                                      sum(Ahal_anno$context=="stop_codon"),
                                      sum(Ahal_anno$context=="start_codon"),
                                      sum(Ahal_anno$context=="transcription_end_site"),
                                      sum(Ahal_anno$context=="intron"),
                                      sum(Ahal_anno$context=="exon"),
                                      sum(Ahal_anno$context=="transcription_start_site")))
  ...
  lyr_anno <- data.frame(category = c("CDS", 
                         ...
                         count = c(sum(Alyr_anno$context=="CDS"),
                                   sum(Alyr_anno$context=="stop_codon"),
                                   sum(Alyr_anno$context=="start_codon"),
                                   sum(Alyr_anno$context=="transcription_end_site"),
                                   sum(Alyr_anno$context=="intron"),
                                   sum(Alyr_anno$context=="exon"),
                                   sum(Alyr_anno$context=="transcription_start_site")))
